<div class="sro-item-detail sro-item-slot-{{ $item->Slot }} {{ $item->ItemInfo->SoxType != 'Normal' ? 'sro-item-special' : '' }}">
    <div class="item" data-itemInfo="1">
        ...
        @endif
        <img alt="" src="{{ asset('images/sro/'.$item->ImgPath) }}" @if(config("settings.item_stats_jid_{$data->user->UserJID}") && !auth()->user()?->role?->is_admin) style="filter: blur(2px); z-index: 5" @endif>
        @if($item->Amount)
